...
const toBounds = ([a, b]: Pair<GridVector2D>): GridBounds2D =>
    GridBounds2D.inclusive(a, b);
...
class Polygon {
    static #toEntries(
        set: Set<number>,
    ): IteratorObject<[number, Array<Pair<number>>]> {
    ...
    includes({ q, r }: GridVector2DRecord): boolean {
        const { vertical } = this;
        if (
            vertical.get(q)?.some(([min, max]) => r >= min && r <= max) ??
            false
        ) {
            return true;
        }
