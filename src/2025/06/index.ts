...
const toEvaluator = (op: Operation): OperationEvaluator =>
    op === Operation.Add ? add : mul;
...
const solve = (
    { values, operations }: Worksheet,
    col: ColumnGenerator,
): number =>
    sum(
        operations
            .entries()
            .map(([index, op]) => col(values, index).reduce(toEvaluator(op))),
    );
