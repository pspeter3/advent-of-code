import { GridBounds2D, GridVector2D, GridVector2DSet } from "../../common/grid2d.ts";
...
const canReach = (bounds: GridBounds2D, rolls: GridVector2DSet, roll: GridVector2D): boolean =>
  len(roll.allNeighbors().filter(([_, n]) => bounds.includes(n) && rolls.has(n))) < 4;

const removeRolls = (bounds: GridBounds2D, rolls: GridVector2DSet): GridVector2DSet =>
  new GridVector2DSet(
    bounds,
    rolls.values().filter((roll) => canReach(bounds, rolls, roll)),
  );

const part1 = ({ bounds, rolls }: Problem): number => removeRolls(bounds, rolls).size;
