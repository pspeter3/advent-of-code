...
const findTarget = ({ target, buttons, joltages }: Machine): number => {
  const options = new Set(
    buttons
      .values()
      .map((set) => parseBinary(Array.from(joltages, (_, i) => (set.has(i) ? "1" : "0")).join(""))),
  );
...
const findJoltage = ({ buttons, joltages }: Machine): number => {
  const model: Model = {
    direction: "minimize",
    objective: "presses",
    constraints: joltages.values().map((joltage, index) => [`c${index}`, equalTo(joltage)]),
...
const parse = (input: string): MachineList => input.trim().split("\n").map(parseMachine);

const part1 = (machines: MachineList): number => sum(machines.values().map(findTarget));

const part2 = (machines: MachineList): number => sum(machines.values().map(findJoltage));
