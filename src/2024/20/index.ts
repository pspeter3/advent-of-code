...
const findCheats = ({ bounds, path }: RaceTrack, radius: number, threshold: number): number => {
  let count = 0;
...
const part1 = (track: RaceTrack): number => findCheats(track, 2, track.path.size === 85 ? 20 : 100);
