...
class DiskChunk {
  ...
  moveFile(chunk: DiskChunk): void {
    ...
    if (this.#space() < delta) {
      throw new Error("Cannot move file to a chunk with insufficient space");
    }
    this.#files.set(id, delta);
    chunk.#files.delete(id);
  }
...
const toDisk = (diskMap: DiskMap): ReadonlyArray<DiskChunk> =>
  diskMap.map((size, index) =>
    index % 2 === 0 ? DiskChunk.fromFile(size, index / 2) : DiskChunk.empty(size),
  );

const defragBlocks = (disk: ReadonlyArray<DiskChunk>): ReadonlyArray<DiskChunk> => {
  let open = 1;
...
const defragFiles = (disk: ReadonlyArray<DiskChunk>): ReadonlyArray<DiskChunk> => {
...
const checksum = (disk: ReadonlyArray<DiskChunk>): number =>
  sum(enumerate(disk.values().flatMap((chunk) => chunk.values())).map(([index, id]) => index * id));

const parse = (input: string): DiskMap => DiskMapSchema.parse(input);

const part1 = (diskMap: DiskMap): number => checksum(defragBlocks(toDisk(diskMap)));

const part2 = (diskMap: DiskMap): number => checksum(defragFiles(toDisk(diskMap)));
