...
const toDirections = (vectors: ReadonlyArray<GridVector2D>): string =>
  vectors
    .map((v) => {
      ...
      const char = isVertical ? (isPositive ? "v" : "^") : isPositive ? ">" : "<";
...
const findKeyPaths = (
  ...
  const result: ReadonlySet<string>[] = [toKeypadPathSet(keypad, "A", code[0] as KeypadKey)];
  for (let i = 1; i < code.length; i++) {
    result.push(toKeypadPathSet(keypad, code[i - 1] as KeypadKey, code[i] as KeypadKey));
  }
  return result;
};

const findSequence = memoize((code: string, depth: number, limit: number): number => {
  if (depth > limit) {
    return code.length;
  }
  let size = 0;
  for (const paths of findKeyPaths(depth === 0 ? NumericKeypad : DirectionalKeypad, code)) {
    size += paths
      .values()
...
const part1 = (codes: ReadonlyArray<string>): number =>
  sum(codes.values().map((code) => findSequence(code, 0, 2) * parseInt(code.slice(0, -1), 10)));

const part2 = (codes: ReadonlyArray<string>): number =>
  sum(codes.values().map((code) => findSequence(code, 0, 25) * parseInt(code.slice(0, -1), 10)));
