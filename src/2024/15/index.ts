...
const parse = (input: string): Warehouse => {
  ...
  const moves = chunks[1].split("\n").flatMap((line) =>
    line
      .trim()
      .split("")
      .map((char) => MOVES.indexOf(char as unknown as (typeof MOVES)[number]) as CardinalDirection),
  );
...
class WideWarehouse {
  ...
  #moveHorizontal(start: GridVector2D, direction: CardinalDirection): ReadonlySet<GridVector2D> {
    let current = start;
    const boxes = new GridVector2DSet(this.bounds);
  ...
  #moveVertical(start: GridVector2D, direction: CardinalDirection): ReadonlySet<GridVector2D> {
