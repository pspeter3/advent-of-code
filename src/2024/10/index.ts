...
import { GridVector2D, GridVector2DSet, MatrixGrid } from "../../common/grid2d.ts";
...
const isValidStep = (grid: MatrixGrid<number>, curr: GridVector2D, next: GridVector2D): boolean =>
  grid.bounds.includes(next) && grid.at(next) - grid.at(curr) === 1;

const findPeaks = (grid: MatrixGrid<number>, trailhead: GridVector2D): number => {
