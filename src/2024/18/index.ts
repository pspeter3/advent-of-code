...
const isExample = (vectors: ReadonlyArray<GridVector2D>): boolean => vectors.length === 25;
...
const explore = (bounds: GridBounds2D, walls: ReadonlySet<GridVector2D>): number | null => {
...
const part1 = ({ bounds, vectors }: Puzzle): number => {
  const result = explore(
    bounds,
    new GridVector2DSet(bounds, vectors.values().take(isExample(vectors) ? 12 : 1024)),
  );
  if (result === null) {
