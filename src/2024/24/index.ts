...
const InitialValueSchema = z
    ...
    .pipe(
        z.tuple([
            z.string(),
            IntSchema.pipe(z.union([z.literal(0), z.literal(1)])),
        ]),
    );
...
const GatesSchema = LinesSchema(GatePairSchema)
    ...
    .transform(
        (pairs) => new Map(pairs.values().map(([gate, key]) => [key, gate])),
    );
...
class Circuit {
    ...
    #value(id: string, path: ReadonlySet<string>): WireValue {
        ...
        this.#values.set(
            ...
            Circuit.#evaluate(
                gate.op,
                this.#value(gate.lhs, next),
                this.#value(gate.rhs, next),
            ),
        );
        return this.#values.get(id)!;
    }
