...
const invert = (target: number, op: Operation, known: number, leftX: boolean): number => {
  switch (op) {
    case Operation.Add: {
      return target - known;
    }
...
const search = (dc: Context<boolean>, ec: Context<number>, key: string, target: number): number => {
...
const part1 = (monkeys: MonkeyMap): number => evaluate({ monkeys, cache: new Map() }, "root");
