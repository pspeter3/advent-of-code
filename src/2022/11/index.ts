...
type Operation = readonly [a: OperationValue, command: OperationCommand, b: OperationValue];
...
const parseStart = (line: string): ReadonlyArray<number> => line.split(", ").map(toInt);

const parseTest = (denominatorLine: string, successLine: string, failureLine: string): Test => {
  ...
  if (denominatorMatch === null || successMatch === null || failureMatch === null) {
  ...
  for (let i = 0; i < limit; i++) {
    for (const [index, inventory] of inventories.entries()) {
      while (inventory.length > 0) {
        ...
        const target = next % test.denominator === 0 ? test.success : test.failure;
...
const part2 = (monkeys: ReadonlyArray<Monkey>): number => {
  const base = monkeys.reduce((product, { test }) => product * test.denominator, 1);
