...
const BLIZZARDS = new Set([Blizzard.Up, Blizzard.Right, Blizzard.Down, Blizzard.Left]);

const isBlizzard = (char: string): char is Blizzard => BLIZZARDS.has(char as Blizzard);
...
class Forecast {
  readonly blizzards: Readonly<Record<Blizzard, ReadonlyArray<ReadonlySet<number>>>>;

  constructor(blizzards: Readonly<Record<Blizzard, ReadonlyArray<ReadonlySet<number>>>>) {
  ...
  for (const [r, line] of lines.entries()) {
    for (const [q, char] of Array.from(line).entries()) {
      if (isBlizzard(char)) {
        const x = q - 1;
        const y = r - 1;
        const isVertical = char === Blizzard.Up || char === Blizzard.Down;
...
const traverse = (board: Tile, forecast: Forecast, state: State, target: Tile): State => {
