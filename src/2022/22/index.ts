...
class Bounds {
  ...
  step(value: number, amount: -1 | 1) {
    if (!this.contains(value)) {
      throw new Error(`Invalid value ${value} for bounds (${this.min}, ${this.max})`);
    }
...
const score = ({ q, r }: Tile, facing: Facing): number => 1000 * (r + 1) + 4 * (q + 1) + facing;

const toAmount = (turn: Turn): number => (turn === Turn.Left ? 3 : 1);

const rotate = (facing: Facing, amount: number): Facing => ((facing + amount) % 4) as Facing;
    ...
    for (let r = 0; r < scaledSize.r; r++) {
      ...
      for (let q = 0; q < scaledSize.q; q++) {
        ...
        tiles.push(
          lines
            .slice(row, row + scale)
            .map((line) => Array.from(line.slice(col, col + scale)) as TileKind[]),
        );
      }
    }
    const sideId = 0;
  ...
  move(count: number): void {
    for (let i = 0; i < count; i++) {
      ...
      if (!this.#inBounds(nextTile)) {
        const neighbor = this.#sides[this.#sideId].neighbor(this.#facing);
        const index = this.#sides.findIndex((tile) => tile.equals(neighbor));
        if (index !== -1) {
          nextSideId = index as SideId;
          nextTile = new Tile(modulo(nextTile.q, this.scale), modulo(nextTile.r, this.scale));
        } else {
          const connection = this.#connections.get(this.#sideId)?.get(this.#facing);
          if (connection === undefined) {
            throw new Error(`Undefined connection for ${this.#sideId} ${this.#facing}`);
          }
