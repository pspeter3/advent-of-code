...
class GridCell {
  ...
  moves({ q, r }: GridCell): MoveList {
    const moves: Move[] = [];
    if (this.q !== q) {
      moves.push([q < this.q ? Direction.Left : Direction.Right, Math.abs(this.q - q)]);
    }
    if (this.r !== r) {
      moves.push([r < this.r ? Direction.Up : Direction.Down, Math.abs(this.r - r)]);
    }
    return moves;
  }
...
const chase = (head: GridCell, tail: GridCell): GridCell => {
  ...
  switch (moves.length) {
    ...
    case 2: {
      const magnitude = moves.reduce((sum, [_, amount]) => sum + amount, 0);
      if (magnitude < 2) {
        throw new Error(`Invalid step ${moves}`);
      }
      if (magnitude > 2) {
