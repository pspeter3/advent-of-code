...
const CAMEL_CARDS = ["2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K", "A"] as const;
type CamelCard = (typeof CAMEL_CARDS)[number];
type CamelHand = readonly [CamelCard, CamelCard, CamelCard, CamelCard, CamelCard];
...
function toHandType(counts: CamelHandCounts): CamelHandType {
  ...
  switch (most) {
    ...
    case 3: {
      return next === 2 ? CamelHandType.FullHouse : CamelHandType.ThreeOfAKind;
    }
    case 2: {
...
type CamelRoundWithTypeSorter = (a: CamelRoundWithType, b: CamelRoundWithType) => number;
...
const parse = (input: string): CamelRoundList => CamelRoundListSchema.parse(input);
...
const part2 = (rounds: CamelRoundList): number =>
  calculateWinnings(
    ...
    createRoundSorter((card) => (card === "J" ? -1 : CAMEL_CARDS.indexOf(card))),
  );
