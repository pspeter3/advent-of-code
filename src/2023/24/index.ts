...
const solve2D = (rays: ReadonlyArray<Ray2D>, range: NumberRange): number =>
  len(
    pairs(rays)
      ...
      .filter((vector) => vector !== null && range.includes(vector.x) && range.includes(vector.y)),
  );
...
const findComponents = (
  ...
  new Map(
    Map.groupBy(rays, ({ velocity }) => velocity[key])
      ...
      .map(([k, list]) => [
        k,
        Array.from(pairs(list), ([{ position: a }, { position: b }]) => a[key] - b[key]),
      ]),
  );

function findComponentVelocity(components: ReadonlyMap<number, ReadonlyArray<number>>): number {
...
function findPosition([a, b]: ReadonlyArray<Ray3D>, velocity: Vector3D): Vector3D {
...
const part2 = (rays: ReadonlyArray<Ray3D>): number =>
  sum(
    Object.values(
      findPosition(rays, rays.length === 5 ? { x: -3, y: 1, z: 2 } : findVelocity(rays)),
    ),
  );
