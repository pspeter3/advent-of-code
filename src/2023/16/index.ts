...
const Schema = LinesSchema(TileListSchema).transform((matrix) => new MatrixGrid(matrix));
...
function energize(grid: MatrixGrid<Tile>, start: Beam): number {
  ...
  const check = (position: GridVector2DRecord, direction: CardinalDirection): boolean => {
  ...
  for (const [position, direction] of queue) {
    ...
    const push = (direction: CardinalDirection): unknown => queue.push([next, direction]);
    switch (grid.at(next)) {
      case ".": {
        push(direction);
        break;
