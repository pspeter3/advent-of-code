...
const parseKey = (key: string): Pick<PulseModuleRecord, "name" | "kind"> =>
  key.startsWith(PulseModuleKind.FlipFlop) || key.startsWith(PulseModuleKind.Conjuction)
...
class ConjuctionPulseModule extends PulseModule {
  ...
  constructor(name: string, outputs: ReadonlyArray<string>, inputs: ReadonlySet<string>) {
    super(name, outputs);
  ...
  process({ source, pulse }: PulseMessage): Pulse | null {
    ...
    return this.#inputs.values().every((p) => p === Pulse.High) ? Pulse.Low : Pulse.High;
  }
}
...
class PulseSystem {
  ...
  static #buildInputs(records: PulseModuleRecordList): ReadonlyMap<string, ReadonlySet<string>> {
  ...
  static #buildModule(
    ...
    switch (kind) {
      ...
      case PulseModuleKind.Conjuction:
        return new ConjuctionPulseModule(name, outputs, inputs.get(name)!);
      default:
  ...
  *press(): Iterable<PulseMessage> {
    const queue: PulseMessage[] = [{ source: "button", target: "broadcaster", pulse: Pulse.Low }];
...
const parse = (input: string): PulseModuleRecordList => PulseModuleRecordListSchema.parse(input);

const part1 = (records: PulseModuleRecordList): number => simulate(new PulseSystem(records), 1_000);
...
const part2 = (records: PulseModuleRecordList): number => {
  ...
  if (records.find(({ name }) => name === target)?.kind !== PulseModuleKind.Conjuction) {
  ...
  return cycles.values().reduce((lcm, cycle) => leastCommonMultiple(lcm, cycle), 1);
};
