    ...
    Map.groupBy(rocks, group)
      ...
      .map((line) => {
        ...
        for (const rock of list) {
          result.push(rock.kind === RockKind.Cube ? rock : shift(result.at(-1), rock));
        }
        return result;
      }),
  ).flat(),
...
const cycle = (platform: Platform): Platform => SPIN_CYCLE.reduce((p, next) => next(p), platform);
...
function findCycle(platform: Platform): PlatformCycle {
  ...
  for (const i of count()) {
    ...
    if (entry) {
      ...
      const platforms = Array.from({ length: period }, (_, j) => index.get(offset + j)!);
