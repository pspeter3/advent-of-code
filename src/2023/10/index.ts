...
const TileMarkerSchema = z.union([z.literal("."), z.literal("S"), PipeMarkerSchema]);
...
function isPipeConnection(positions: ReadonlyArray<GridPosition>): positions is PipeConnection {
  return positions.length === 2;
}

function toConnection(position: GridPosition, marker: PipeMarker): PipeConnection {
  switch (marker) {
...
function findStartConnections(grid: TileGrid, start: GridPosition): PipeConnection {
  const connection = start.neighbors().filter((position) => {
    ...
    return toConnection(position, marker).some((position) => position.equals(start));
  });
...
function* walkLoop(grid: TileGrid, start: GridPosition, next: GridPosition): Iterable<LoopEntry> {
  let count = 0;
...
const COLINEAR: ReadonlySet<TileMarker> = new Set([PipeMarker.SouthWest, PipeMarker.NorthEast]);

function isInside(grid: TileGrid, border: ReadonlySet<number>, position: GridPosition): boolean {
  let count = 0;
  let current = position;
  while (hasPosition(grid, current)) {
    if (border.has(current.toId(grid)) && !COLINEAR.has(toMarker(grid, current))) {
      count++;
    }
...
const part1 = (grid: TileGrid): number => {
  ...
  const [a, b] = findStartConnections(grid, start).map(
    (next) => new Map(walkLoop(grid, start, next) as ReadonlyMap<number, number>),
  );
  let max = -Infinity;
...
const part2 = (grid: TileGrid): number => {
  ...
  const border: ReadonlySet<number> = new Set(new Map(walkLoop(grid, start, next)).keys());
  let count = 0;
  for (const [r, row] of grid.entries()) {
