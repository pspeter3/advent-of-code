...
import { CardinalDirection, GridVector2D, MatrixGrid } from "../../common/grid2d.ts";
  ...
  for (const curr of queue) {
    ...
    const options = Array.from(
      curr
        ...
        .map(([_, n]) => n)
        .filter(
          (n) => !queue.at(-2)!.equals(n) && grid.bounds.includes(n) && !isForest(grid.at(n)),
        ),
    );
    if (options.length !== 1) {
...
function toWeightGraph(grid: MatrixGrid<Tile>, start: GridVector2D, slopes: boolean): WeightGraph {
...
const toStart = ({ bounds }: MatrixGrid<unknown>): GridVector2D => bounds.min.add({ q: 1, r: 0 });
const toEnd = ({ bounds }: MatrixGrid<unknown>): GridVector2D => bounds.max.add({ q: -2, r: -1 });
...
function solve(grid: MatrixGrid<Tile>, slopes: boolean): number {
  ...
  return longestPath(graph, grid.bounds.toId(start), grid.bounds.toId(toEnd(grid)), new Set());
}
