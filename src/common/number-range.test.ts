...
describe("NumberRange", () => {
  ...
  it("should support intersection", () => {
    assert.deepEqual(new NumberRange(1, 5).intersection({ min: 2, max: 6 }), new NumberRange(2, 5));
    assert.deepEqual(new NumberRange(1, 5).intersection({ min: 5, max: 10 }), null);
    assert.deepEqual(new NumberRange(1, 5).intersection({ min: 0, max: 1 }), null);
  });

  it("should support split", () => {
    const range = new NumberRange(1, 5);
    assert.throws(() => range.split(0));
    assert.deepEqual(range.split(3), [new NumberRange(1, 3), new NumberRange(3, 5)]);
  });
