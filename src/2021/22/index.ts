...
const inBounds = (cuboid: Cuboid): boolean => cuboid.every(([min, max]) => min >= -50 && max <= 50);

const increment = (counter: Map<string, number>, key: string, amount: number = 1): void => {
  counter.set(key, (counter.get(key) ?? 0) + amount);
};

const reboot = (steps: ReadonlyArray<Step>, filter: (cuboid: Cuboid) => boolean): number => {
