...
type Input = readonly [template: string, replacements: ReadonlyArray<Replacement>];

const increment = (counts: Map<string, number>, key: string, amount: number): Map<string, number> =>
...
const replace = (
  ...
  for (const [key, count] of tokens) {
    ...
    const pairs = key
      .split("")
      .map((char, i) => (i === 0 ? [char, insert] : [insert, char]).join(""));
