...
const WAITING: ReadonlyArray<number> = Array.from({ length: 11 }, (_, i) => i).filter(
  (i) => !DOORS.includes(i),
);
...
const findOptions = (
...
): readonly [moves: ReadonlyArray<Cell | null>, slots: ReadonlyArray<Cell | null>] => {
  ...
  for (const [line, room] of rooms.entries()) {
    ...
    const isOpen = room.every((node) => node === null || node === AMPHIPODS[line]);
    if (isOpen) {
      moves.push(null);
...
const createFinal = (size: number): Grid =>
  AMPHIPODS.map((amphipod: Amphipod): Line => Array.from({ length: size }, () => amphipod)).concat([
    createHall(),
  ]);
...
const schema = z.preprocess(
  (input) => {
    const data = StringSchema.parse(input)
      ...
      .split("\n")
      .map((line) => line.split(""));
    return [...DOORS.map((door) => [data[2][door + 1], data[3][door + 1]]), createHall()];
  },
