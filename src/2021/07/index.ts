...
const schema = z.preprocess((input) => StringSchema.parse(input).split(","), z.array(IntSchema));

const identity = (distance: number): number => distance;
const triangle = (distance: number): number => 0.5 * distance * (distance + 1);

const fuel = (crabs: ReadonlyArray<number>, cost: (distance: number) => number): number =>
  Math.min(
    ...Uint32Array.from({ length: Math.max(...crabs) - Math.min(...crabs) + 1 }, (_, index) =>
      crabs.reduce((sum, crab) => sum + cost(Math.abs(crab - index)), 0),
    ),
  );
