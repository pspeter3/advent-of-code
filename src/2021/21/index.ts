...
const move = (position: number, delta: number): number => ((position + delta - 1) % 10) + 1;
...
const part1 = (positions: Pair): number => {
  ...
  while (Math.max(...scores) < 1000) {
    ...
    const delta = Array.from({ length: 3 }, (_, v) => ((3 * turn + v) % 100) + 1).reduce(
      (sum, value) => sum + value,
    );
...
const evolve = (turn: number, [positions, scores]: Game, delta: number): string => {
...
const part2 = (positions: Pair): number => {
  let wins: Pair = [0, 0];
  let games: ReadonlyMap<string, number> = new Map([[serialize([positions, [0, 0]]), 1]]);
