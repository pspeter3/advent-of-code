...
type Input = readonly [vectors: ReadonlyArray<Vector>, folds: ReadonlyArray<Fold>];
type Key = string;
...
const part1 = ([vectors, folds]: Input): number => fold(toDots(vectors), folds[0]).size;
...
const part2 = ([vectors, folds]: Input): string => {
  ...
  return Array.from({ length: rows + 1 }, (_, y) =>
    Array.from({ length: cols + 1 }, (_, x) => (dots.has(serialize([x, y])) ? "#" : ".")).join(""),
  ).join("\n");
};
