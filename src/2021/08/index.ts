...
const schema = LinesSchema(
  z.preprocess((line) => StringSchema.parse(line).split(" | "), z.tuple([Signal, Signal])),
);

const fromDigits = (digits: ReadonlyArray<number>): number =>
  digits.reduce((sum, digit, index) => sum + Math.pow(10, digits.length - index - 1) * digit, 0);

const sorted = (wire: string): string => Array.from(wire).sort().join("");

const find = (wires: Set<string>, callback: (wire: string) => boolean): string => {
...
const analyze = (signal: Signal): ReadonlyMap<string, number> => {
  ...
  results[6] = find(wires, (wire) => wire.length === 6 && !isSuperset(new Set(wire), one));
  results[9] = find(wires, (wire) => {
    const chars = new Set(wire);
    return wire.length === 6 && isSuperset(chars, four) && isSuperset(chars, seven);
  });
...
const part1 = (data: ReadonlyArray<Entry>): number => {
  ...
  return data.reduce(
    (sum, [_, output]) => sum + output.filter((value) => unique.has(value.length)).length,
    0,
  );
};
