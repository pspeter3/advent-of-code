...
const schema = LinesSchema(
  z.preprocess((line) => StringSchema.parse(line).trim().split(""), z.array(z.enum(Cell))),
);
...
const prev = (index: number, max: number): number => (index === 0 ? max - 1 : index - 1);
...
const evolve = (grid: Grid): Grid => {
  ...
  const east = grid.map((row) =>
    row.map((cell, index) => {
      switch (cell) {
        case Cell.East: {
          return row[next(index, cols)] === Cell.Empty ? Cell.Empty : cell;
        }
        case Cell.South: {
          return cell;
        }
        case Cell.Empty: {
          return row[prev(index, cols)] === Cell.East ? Cell.East : cell;
        }
      }
    }),
  );
  return east.map((row, r) =>
    row.map((cell, c) => {
      switch (cell) {
        ...
        case Cell.South: {
          return east[next(r, rows)][c] === Cell.Empty ? Cell.Empty : cell;
        }
        case Cell.Empty: {
          return east[prev(r, rows)][c] === Cell.South ? Cell.South : cell;
        }
      }
    }),
  );
