...
const schema = LinesSchema(
  z.preprocess((line) => StringSchema.parse(line).split(""), z.array(CharactersSchema)),
);
...
const isCloser = (char: Character): char is Closer => CloseSchema.safeParse(char).success;
    ...
    if (!corrupted && stack.length > 0) {
      scores.push(
        stack
          .reverse()
          .reduce(
            (score, char) => 5 * score + IncompleteScores[Closers[OpenSchema.parse(char)]],
            0,
          ),
      );
    }
