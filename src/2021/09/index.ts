...
const schema = LinesSchema(
  z.preprocess((line) => StringSchema.parse(line).trim().split(""), z.array(IntSchema)),
);

type Position = readonly [row: number, col: number];

const neighbors = ([rows, cols]: Position, [row, col]: Position): ReadonlyArray<Position> => {
...
const toID = ([_, cols]: Position, [row, col]: Position): number => cols * row + col;

const size = (uphill: ReadonlyMap<number, ReadonlySet<number>>, basin: number): number => {
  if (!uphill.has(basin)) {
...
const part2 = (data: ReadonlyArray<ReadonlyArray<number>>): number => {
  ...
  const results = basins.map((basin) => size(uphill, basin)).sort((a, b) => b - a);
