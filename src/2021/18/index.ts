...
const PairSchema: z.ZodSchema<Pair> = z.lazy(() =>
  z.tuple([z.union([PairSchema, z.number().int()]), z.union([PairSchema, z.number().int()])]),
);
...
const traverse = (
  ...
  let index = 0;
  const invoke = (node: Node, depth: number): Node => callback(node, depth, index++);
...
const findExplode = (elements: ReadonlyArray<Element>): ExplodeAction | null => {
  const indices: number[] = [];
  let result: readonly [pair: Pair, index: number, count: number] | null = null;
