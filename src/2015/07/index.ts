...
type CommandInstruction = readonly [a: InstructionValue, command: Command, b: InstructionValue];
...
const NotInstructionSchema = z.tuple([z.literal("NOT"), InstructionValueSchema]);
...
const CircuitSchema = LinesSchema(InstructionPairSchema).transform((pairs) => new Map(pairs));

const parse = (input: string): Circuit => CircuitSchema.parse(input);

const isValueInstruction = (instruction: Instruction): instruction is ValueInstruction =>
  instruction.length === 1;

const isNotInstruction = (instruction: Instruction): instruction is NotInstruction =>
